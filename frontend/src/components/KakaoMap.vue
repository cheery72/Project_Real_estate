<template>
    <div>
        <div id="map"></div>
        <b-button v-b-toggle.sidebar-right ref="myButton" style="display:none;"></b-button>
...
export default {
    ...
    methods: {
        ...
        initMap() {
            ...
            this.aptImage = new kakao.maps.MarkerImage(
                aptSrc,
                aptSize,
                aptOption
            );
            ...
            this.cctvImage = new kakao.maps.MarkerImage(
                cctvSrc,
                cctvSize,
                cctOption
            );
            ...
            this.subwayImage = new kakao.maps.MarkerImage(
                subwaySrc,
                subwaySize,
                subwayOption
            );
            ...
            this.schoolImage = new kakao.maps.MarkerImage(
                schoolSrc,
                schoolSize,
                schoolOptoin
            );
        ...
        displayMarker(markerPositions) {
            ...
            var circle; //현재 선택한 곳만 반경을 찍어주기 위해 전역으로 선언.
            var mar;
            var count = 0;

            if (markerPositions.length > 0) {
                markerPositions.forEach((position) => {
                    const latln = new kakao.maps.LatLng(
                        position.lat,
                        position.lng
                    );
                    ...
                    kakao.maps.event.addListener(marker, "click", () => {
                        ...
                        mar = marker.getPosition();
                        circle.setOptions({ center: mar });
                        // 지도에 원을 표시합니다
                        ...
                        this.$store.state.cctvCount = count;
