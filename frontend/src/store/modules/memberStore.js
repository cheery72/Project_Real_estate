...
const memberStore = {
    ...
    actions: {
        ...
        getBookmark({ commit }, mark) {
            ...
            const params = {
                bookmark: mark.aptCode,
                accesstoken: decode_token.userid,
            };

            http.post(`/bookmark/register`, params)
                .then((response) => {
        ...
        getBookmarkList({ commit }) {
            ...
            http.get(`/bookmark/list/${userid}`).then(({ data }) => {
                commit("SET_BOOKMARK_LIST_INFO", data.result);
            });
        },
    },
};
